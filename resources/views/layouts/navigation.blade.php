<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(in_array(Auth::user()->role, ['trainee', 'super_admin', 'admin']))
                        <x-nav-link :href="Auth::user()->role === 'trainee' ? route('trainee.profile.edit') : route('profile.edit')" :active="request()->routeIs('trainee.profile.*') || request()->routeIs('profile.edit')">
                            {{ __('My profile') }}
                        </x-nav-link>
                    @endif
                    ...
                    @if(Auth::user()->role === 'super_admin')
                        ...
                        <x-nav-link :href="route('tmx-auction.index')" :active="request()->routeIs('tmx-auction.*')">
                            {{ __('TMX Auction Data') }}
                        </x-nav-link>
                    @endif
                    --}}
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(in_array(Auth::user()->role, ['trainee', 'super_admin', 'admin']))
                <x-responsive-nav-link :href="Auth::user()->role === 'trainee' ? route('trainee.profile.edit') : route('profile.edit')" :active="request()->routeIs('trainee.profile.*') || request()->routeIs('profile.edit')">
                    {{ __('My profile') }}
                </x-responsive-nav-link>
            @endif
            {{-- WRMS / TMX modules hidden for now
            @if(Auth::user()->role === 'super_admin')
                ...
                <x-responsive-nav-link :href="route('tmx-auction.index')" :active="request()->routeIs('tmx-auction.*')">
                    {{ __('TMX Auction Data') }}
                </x-responsive-nav-link>
            @endif
            --}}
